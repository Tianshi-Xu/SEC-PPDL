...
MatmulCtpt::MatmulCtpt(uint64_t dim_0, const Tensor<uint64_t>& weight, const Tensor<uint64_t>& bias, HE::HEEvaluator* HE)
    : dim_0(dim_0), 
      weight(weight), 
      bias(bias), 
      HE(HE)
...
MatmulCtpt::MatmulCtpt(uint64_t dim_0, uint64_t dim_1, uint64_t dim_2, HE::HEEvaluator* HE)
    : dim_0(dim_0),
      dim_1(dim_1),
      dim_2(dim_2),
      HE(HE)
...
MatmulCtptBolt::MatmulCtptBolt(uint64_t dim_0, const Tensor<uint64_t>& weight, const Tensor<uint64_t>& bias, HE::HEEvaluator* HE)
    : MatmulCtpt(dim_0, weight, bias, HE)
{
...
MatmulCtptBolt::MatmulCtptBolt(uint64_t dim_0, uint64_t dim_1, uint64_t dim_2, HE::HEEvaluator* HE)
    : MatmulCtpt(dim_0, dim_1, dim_2, HE)
{
    padded_dim_0 = dim_0 - 1;
    for (int i = 0; i < 5; i++) {
        padded_dim_0 |= padded_dim_0 >> (1 << i);
...
Tensor<UnifiedPlaintext> MatmulCtptBolt::PackWeight() {
...
Tensor<uint64_t> MatmulCtptBolt::PackActivation(Tensor<uint64_t> &x) {
...
Tensor<UnifiedCiphertext> MatmulCtptBolt::HECompute(const Tensor<UnifiedPlaintext> &weight_pt, Tensor<UnifiedCiphertext> &ac_ct) {
    /** 
     *  NOTE:
...
Tensor<uint64_t> MatmulCtptBolt::DepackResult(Tensor<uint64_t> &out_msg) {
...
Tensor<uint64_t> MatmulCtptBolt::operator()(Tensor<uint64_t> &x) {
...
Tensor<uint64_t> MatmulCtctBumble::operator()(Tensor<uint64_t> &x1, Tensor<uint64_t> &x2) {
    ...
    MatmulCtptBolt* ctpt_mult_1 = new MatmulCtptBolt(x1.shape()[0], x2, x2, this->HE);
    Tensor<uint64_t> cross_term_1 = ctpt_mult_1->operator()(x1);
    std::cout << "Cross term 1 generated" << std::endl;
    MatmulCtptBolt* ctpt_mult_2 = new MatmulCtptBolt(x2_t.shape()[0], x1_t, x1_t, this->HE);
