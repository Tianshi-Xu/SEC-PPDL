...
namespace {
constexpr uint32_t kDModulusStrideWords = static_cast<uint32_t>(sizeof(DModulus) / sizeof(uint64_t));
static_assert(sizeof(DModulus) % sizeof(uint64_t) == 0, "DModulus layout must be uint64_t-aligned.");
}

__global__ void negacyclic_shift_kernel(const uint64_t *src_data,
                                        uint64_t *dst_data,
                                        const uint32_t coeff_count,
                                        const uint32_t shift,
                                        const uint64_t *moduli_device) {
    const uint32_t coeff_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (coeff_idx >= coeff_count) {
        return;
    }

    const uint32_t mod_idx = blockIdx.y;
    const uint32_t poly_idx = blockIdx.z;
    const uint32_t coeff_mod_count = gridDim.y;
    const uint32_t shift_mod = shift % (coeff_count << 1);
    const uint32_t raw = coeff_idx + shift_mod;
    const uint32_t dst_coeff_idx = raw % coeff_count;
    const uint32_t wraps = raw / coeff_count;

    const size_t poly_stride = static_cast<size_t>(coeff_mod_count) * coeff_count;
    const size_t coeff_base = static_cast<size_t>(poly_idx) * poly_stride + static_cast<size_t>(mod_idx) * coeff_count;
    uint64_t value = src_data[coeff_base + coeff_idx];

    if (wraps & 1U) {
        const uint64_t modulus = moduli_device[static_cast<size_t>(mod_idx) * kDModulusStrideWords];
        const uint64_t non_zero = (value != 0);
        value = (modulus - value) & static_cast<uint64_t>(-static_cast<int64_t>(non_zero));
    }

    dst_data[coeff_base + dst_coeff_idx] = value;
}

void launch_negacyclic_shift_kernel(const uint64_t *src_data,
                                    uint64_t *dst_data,
                                    uint32_t coeff_count,
                                    uint32_t coeff_mod_count,
                                    uint32_t poly_count,
                                    uint32_t shift,
                                    const uint64_t *moduli_device,
    ...
    const dim3 grid((coeff_count + kThreads - 1) / kThreads, coeff_mod_count, poly_count);
    negacyclic_shift_kernel<<<grid, block, 0, stream>>>(src_data, dst_data, coeff_count, shift, moduli_device);
}
